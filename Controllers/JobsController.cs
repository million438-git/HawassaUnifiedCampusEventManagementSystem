...
namespace HawassaUnifiedCampusEventManagementSystem.Controllers
{
    public class JobsController : Controller
    {
        ...
        // =========================================================
        [HttpGet]
        public IActionResult GetJobJson(int id)
        {
            var job = MasterJobsCatalog.FirstOrDefault(j => j.Id == id);
            if (job == null)
            {
        ...
        public IActionResult Apply(JobApplicationViewModel model)
        {
            ...
            var appId = $"APP-HU-{DateTime.UtcNow:yyyyMMdd}-{new Random().Next(1000, 9999)}";
            
            // Record the application in the live list
            SubmittedApplications.Insert(0, new StudentApplicationItemViewModel
            {
                ApplicationId = appId,
                JobId = model.JobId,
                ...
                Notes = $"Application received successfully for {model.FullName} ({model.Department})."
            });
        ...
        // =========================================================
        // POST: /Jobs/QuickApplyJson (From Modal)
        // =========================================================
        [HttpPost]
        public IActionResult QuickApplyJson([FromBody] JobApplicationViewModel model)
        {
            if (string.IsNullOrWhiteSpace(model.FullName) || string.IsNullOrWhiteSpace(model.Email))
            {
                return Json(new { success = false, message = "Please fill in all required fields (Full Name and Email)." });
            }
            ...
            SubmittedApplications.Insert(0, new StudentApplicationItemViewModel
            {
                ...
                Status = "Submitted",
                StatusBadgeClass = "bg-primary-subtle text-primary-emphasis",
                Notes = $"Quick application received for {model.FullName}."
            });
            ...
            return Json(new
            {
                success = true,
                applicationId = appId,
                message = $"Congratulations {model.FullName}! Your application for {model.JobTitle} at {model.CompanyName} has been submitted successfully."
            });
        }
